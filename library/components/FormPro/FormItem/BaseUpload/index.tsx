...
const defaultChildren = (disabled: boolean, listType?: string) => {
...
export const Upload: React.FC<IBaseUploadProps> = (props) => {
  ...
  // 渲染children
  const renderChildren = children || defaultChildren(disabled, listType);
  ...
  const onFileChange = useCallback(
    ({ fileList }: UploadChangeParam<UploadFile<any>>) => {
      ...
      const newList = fileList.filter((item) => {
        const { status, error, response } = item;
        if (response?.code !== '000000' && error) {
